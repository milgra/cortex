...
v2_t v2_mirror(v2_t axis, v2_t vector)
{
    v2_t projected_point = v2_intersect_lines(v2_init(0.0, 0.0), axis, vector, v2_rotate_90_right(axis));
...
v2_t v2_intersect_lines(v2_t transa, v2_t basisa, v2_t transb, v2_t basisb)
{
    if (!(fabs(basisa.x) > 0.0 || fabs(basisa.y) > 0.0)) return v2_init(FLT_MAX, FLT_MAX);
    if (!(fabs(basisb.x) > 0.0 || fabs(basisb.y) > 0.0)) return v2_init(FLT_MAX, FLT_MAX);
...
float v2_endpoint_proximity(v2_t transa, v2_t basisa, v2_t transb, v2_t basisb)
{
    ...
    v2_t isp_inertia_segment = v2_intersect_lines(
	transa,
	basisa,
	transb,
	basisb);
...
v2_t v2_intersect_with_proximity(v2_t trans_a, v2_t basis_a, v2_t trans_b, v2_t basis_b, float proximity)
{
	...
	if (distance < proximity) isp = v2_intersect_lines(
				      trans_a,
				      basis_a,
				      trans_b,
				      basis_b);
...
v2_t v2_triangle_with_bases(v2_t point_a, v2_t point_b, float segmentlength, int8_t direction)
{
	...
	v2_t  distance = v2_init((float) direction * -vector.y, (float) direction * vector.x);
	distance       = v2_resize(distance, needed);
	vector         = v2_add(point_a, vector);
	vector         = v2_add(vector, distance);
    }
    else vector = v2_add(point_a, vector);
    return vector;
}
...
segment2_t v2_collide_and_fragment(v2_t transa, v2_t basisa, v2_t transb, v2_t basisb)
{
    ...
    if (final_length > 0.0) new_basis = v2_resize(new_basis, final_length);
    else new_basis = v2_init(0.0, 0.0);
